...
class ShowCallInstance extends InstanceBase {
	...
	initPresets() {
		...
		// Macro presets for common scenes
		const commonMacros = [
			{ id: 'walkin', label: 'Walk-In Scene', color: 0x0ea5e9 },
			{ id: 'sermon', label: 'Sermon Scene', color: 0x22c55e },
			{ id: 'baptism', label: 'Baptism Scene', color: 0x8b5cf6 },
			{ id: 'closing', label: 'Closing Scene', color: 0xf59e0b }
		]

		commonMacros.forEach(macro => {
			presets.push({
				type: 'button',
				category: 'Macros',
				name: macro.label,
				style: {
					text: macro.label.replace(' Scene', ''),
					size: '12',
					color: 16777215,
					bgcolor: macro.color
				},
